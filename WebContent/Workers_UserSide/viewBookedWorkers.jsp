...
<body>
<h1 align="center">Booked Workers</h1>
...
try{
	...
	PreparedStatement ps = con.prepareStatement("SELECT  book_worker.book_id,workers.wname,workers.type, workers.phone_no FROM ((book_worker INNER JOIN flat_users ON book_worker.email = flat_users.email) INNER JOIN workers ON book_worker.w_id = workers.w_id) where book_worker.email=?");
...
%>
<tr>
<TD><%=rs.getString("book_worker.book_id")%></TD>
<TD><%=rs.getString("workers.wname")%></TD>
<TD><%=rs.getString("workers.type")%></TD>
<TD><%=rs.getString("workers.phone_no")%></TD>





<td><%-- <a href="cancelWorker.jsp?book_id=<%=rs.getString("book_id")%>"> --%><button  onclick="myFunction()" class="btn">Cancel</button></a></td>
</tr>
<script>
function myFunction() {
  var txt;
  ...
  if (r == true) {
    txt = "Confirm";
    window.location = "cancelWorker.jsp?book_id=<%=rs.getString("book_id")%>";
  } else {
    txt = "No";
  }
