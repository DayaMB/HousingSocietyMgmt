...
<%
try{
	PreparedStatement ps = con.prepareStatement("SELECT v_id,flat_users.flat_id,name,comment,date,createdate,status FROM flat_users INNER JOIN visitor_info ON flat_users.email = visitor_info.email where status<>'accepted' and visitor_info.email=?");
...
<%-- <td><a href="updateVisitorForm?v_id=<%=rs.getString("v_id")%>"><button class="btn">Update</button></a></td>
 --%></tr>
<%
i++;
}
